...
def main():
    path = input("Enter .txt file name that to be converted into an image : \n")
    image = textfile_to_image(path)
    image.show()
    rand_str = ''.join(random.choice(string.ascii_letters) for _ in range(5))
    fname = "ascii_image_" + rand_str + ".png"
    image.save(fname)
...
def textfile_to_image(textfile_path):
    ...
    image_width = int(math.ceil(max_line_width + (2 * margin_pixels)) * 0.75) # added 0.75 multiplier 
